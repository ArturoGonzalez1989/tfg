...
<div class="container mt-5">
	<h2 class="text-center">Editar mensaje de: <a href="{{ route('usuarios.show', $mensaje->user->id) }}">{{ $mensaje->user->nombre }}</a> en el punto <a href="{{ route('rutas.show', $mensaje->punto->id) }}">{{ $mensaje->punto->nombre }}</a></h2>
	<hr>
	<form method="POST" id="formulario" action="{{ route('mensajes_puntos.update', $mensaje->id) }}">
	{!! method_field('PUT') !!}	
 {{ csrf_field() }}
</div>
